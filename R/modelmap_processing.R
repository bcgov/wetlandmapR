...
wetland_map <- function (model.out,
  ...
  if (!is.null(aoi)) {
    rs <- stack_rasters(unique(rastLUT[ ,1]),aligned = T)
  ...
  for (i in 1:length(model.out)) {
    ...
    if (is.null(aoi) & length(model.out) > 1) {
      ...
      if (!aoi.target %in% as.character(as.data.frame(aoi)[,aoi.col])) {
        ...
        aoi.target.shp <- aoi[as.logical(aoi[[aoi.col]] == aoi.target),]
        
        # Clip rasters to poly
        rs.crop <- terra::crop(rs, ext(aoi.target.shp))
        rs.mask <- terra::mask(rs.crop, aoi.target.shp)
