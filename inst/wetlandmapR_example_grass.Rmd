...
A docker image based on the [**rocker/geospat:4.0.3**](https://hub.docker.com/r/rocker/geospatial) image has been assembled for running the [wetlandmapR package](https://github.com/bcgov/wetlandmapR) package. Assuming a local installation of Docker, the [huntgdok/geospat](https://hub.docker.com/repository/docker/huntgdok/geospat) image can be downloaded by passing the command: 
...
```{bash eval=FALSE}
docker run -e PASSWORD=APassword -p 8787:8787 --rm huntgdok/geospat:4.0.3
```
