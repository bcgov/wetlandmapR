...
```{bash eval=FALSE}
docker run -e PASSWORD=APassword -p 8787:8787 --rm huntgdok/wetlandmapr:latest
```
...
#Attribute training points with values at intersection with input layers, write results to CSV, and group by provided AOI. 
...
Below we build a random forest wetland classification model by calling the ***wetlandmapR::wetland_model*** function using the training data that we attributed with the input raster layers above. We declare the response field to be the *T_W_Class* field. We specify that we want to use the *BGC_ZONE* column to identify separate AOIs. We also define the *SINKS* column (input) to be a factor variable. The results include a list containing *model.build* objects output from the [ModelMap](https://cran.r-project.org/web/packages/ModelMap/index.html) package for each AOI. 
...
plot(as.factor(raster(paste(tempdir(),'/wetlandmapR_expl/output/',SBS.dir,'/',SBS.img,sep=''))),col=c("blue","brown","black","green","white","pink"),legend=F)
legend("topright", legend = c("Fl","T","Water","Wb","Wf","Ws"), fill = c("blue","brown","black","green","white","pink"))
plot(as.factor(raster(paste(tempdir(),'/wetlandmapR_expl/output/',ESSF.dir,'/',ESSF.img,sep=''))),col=c("brown","white","green"),legend=F)
```
